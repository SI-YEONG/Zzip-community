...
if page == "🏠 챌린지 인증":
    ...
    if mode == "가입하지 않았습니다":
        ...
        if st.button("회원가입 후 루틴 저장"):
            if len(password.strip()) != 4 or not password.strip().isdigit():
                ...
                st.error("이미 존재하는 사용자입니다. 로그인해주세요.")
            else:
        ...
        matched = (user_df["이름"].astype(str).str.strip() == username.strip()) & (user_df["비밀번호"].astype(str).str.strip() == password.strip())
        if login_button:
            if matched.any():
    ...
    if st.button("📝 커뮤니티에 남기기", key="submit_post"):
        if username and post:
            ...
            today = datetime.now().strftime("%Y-%m-%d")
            new_post = pd.DataFrame([{"글ID": post_id, "닉네임": username, "날짜": today, "내용": post}])
    ...
    for _, row in community_df.sort_values(by="날짜", ascending=False).tail(10).iloc[::-1].iterrows():
        ...
        with st.form(f"댓글_{row['글ID']}"):
            ...
            if submitted and commenter and comment:
                new_comment = pd.DataFrame([{"글ID": row["글ID"], "닉네임": commenter, "날짜": datetime.now().strftime("%Y-%m-%d"), "내용": comment}])
    ...
    if username_check == "짱아러버" and password_check == "1234":
        st.success("✅ 관리자 모드입니다.")
        st.subheader("📋 가입된 사용자 목록")
        st.dataframe(user_df)
        st.subheader("📆 전체 인증 로그")
        st.dataframe(log_df)
    else:
